        ...
        for file in files:
            if file.endswith('no_silence.wav'):
                ...
                if in_dev_split(partic_id):
                    ...
                    if depressed:
                        depressed_dict1[partic_id] = fbank
                        depressed_dict2[partic_id] = mfcc
                    elif not depressed:
                        normal_dict1[partic_id] = fbank
...
def create_sample_dicts(crop_width):
    ...
    depressed_samples1 = build_class_sample_dict(depressed_dict1, n_samples,
                                                crop_width)
    depressed_samples2 = build_class_sample_dict(depressed_dict2, n_samples,
                                                 crop_width)
    normal_samples1 = build_class_sample_dict(normal_dict1, n_samples,
                                             crop_width)
    normal_samples2 = build_class_sample_dict(normal_dict2, n_samples,
                                             crop_width)

    for key, _ in depressed_samples1.items():
        path = '/mnt/xlancefs/home/rtg99/data/processed/'
        filename = 'D_fbank{}.npz'.format(key)
        outfile = path + filename
        np.savez(outfile, *depressed_samples1[key])
    ...
    for key, _ in normal_samples1.items():
        path = '/mnt/xlancefs/home/rtg99/data/processed'
        filename = '/N_fbank{}.npz'.format(key)
        outfile = path + filename
        np.savez(outfile, *normal_samples1[key])
